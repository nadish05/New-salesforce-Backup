...
console.log(
    ...
    _mergeLayoutRecordsIntoGroups(
        ...
        const attachAssociatedFiles = (objectGroup, layoutFiles) => {
            const sorted = sortFilesByName(layoutFiles);
            objectGroup.associatedFiles = [
                ...(objectGroup.associatedFiles || []),
                ...sorted
            ];
        ...
        if (customGroup?.objectGroups) {
            customGroup.objectGroups.forEach((objectGroup) => {
                ...
                attachAssociatedFiles(objectGroup, layouts);
                bumpCount(customGroup, layouts.length);
                remaining.delete(objectGroup.objectName);
            });
        }

        const apexGroup = groupedResults.find(
            (group) => group.type === 'ApexClass'
        );
        if (apexGroup) {
            [...remaining.keys()].forEach((objectName) => {
                const layouts = remaining.get(objectName);
                if (!layouts?.length) {
                    return;
                }

                const relatedApex = apexGroup.files.filter((file) =>
                    apexClassBelongsToObject(file, objectName)
                );
                if (!relatedApex.length) {
                    return;
                }

                const relatedApexIds = new Set(
                    relatedApex.map((file) => file.Id)
                );
                apexGroup.files = apexGroup.files.filter(
                    (file) => !relatedApexIds.has(file.Id)
                );

                apexGroup.objectGroups = apexGroup.objectGroups || [];
                let objectGroup = apexGroup.objectGroups.find(
                ...
                if (!objectGroup) {
                    ...
                    apexGroup.objectGroups.push(objectGroup);
                }

                const sortedApex = sortFilesByName(relatedApex);
                const sortedLayouts = sortFilesByName(layouts);
                objectGroup.associatedFiles = [
                    ...sortedApex,
                    ...sortedLayouts
                ];
                ...
                bumpCount(apexGroup, sortedLayouts.length);
                remaining.delete(objectName);
            });

            apexGroup.objectGroups.sort((left, right) =>
                left.objectName.localeCompare(right.objectName)
            );
        }

        const layoutOnlyObjectGroups = [...remaining.entries()].map(
            ([objectName, layouts]) => {
                const sorted = sortFilesByName(layouts);
                remaining.delete(objectName);
                return {
                    objectName,
                    expanded: false,
                    files: sorted,
                    associatedFiles: sorted,
                    metadataGroups: []
                };
            }
        );
        ...
        if (unresolvedLayouts.length > 0) {
            let layoutGroup = groupedResults.find(
                (group) => group.groupKey === 'Layout-unresolved'
            );
            if (!layoutGroup) {
                layoutGroup = {
                    type: 'Layout',
                    groupKey: 'Layout-unresolved',
                    count: 0,
                    expanded: expandedStates['Layout-unresolved'] || false,
                    files: []
                };
                groupedResults.push(layoutGroup);
            }

            layoutGroup.files.push(...unresolvedLayouts);
            layoutGroup.count += unresolvedLayouts.length;
        }

        if (layoutOnlyObjectGroups.length > 0) {
            const objectCount = layoutOnlyObjectGroups.reduce(
                (total, objectGroup) =>
                    total + (objectGroup.associatedFiles?.length || 0),
                0
            );

            groupedResults.push({
                type: 'Layout',
                groupKey: 'Layout-by-object',
                suppressTypeHeader: true,
                count: objectCount,
                expanded: true,
                files: [],
                objectGroups: layoutOnlyObjectGroups.sort((left, right) =>
                    left.objectName.localeCompare(right.objectName)
                )
            });
        }
