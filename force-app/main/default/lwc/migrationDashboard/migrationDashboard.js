...
import executeRollback
from '@salesforce/apex/DeploymentRollbackController.executeRollback';
...
_parseRollbackResponse(raw) {
    ...
    if (parsed.success === true) {
        return { kind: 'success', message, code: parsed.code || null };
    }
...
handleRollback() {
    if (!this.showRollbackPanel || this.isRollingBack || this.rollbackCompleted) {
        return;
    }
...
_extractMetadataName(file) {
    ...
    // CustomMetadata DX files are Type.Record.md-meta.xml. Keep the full
    // Type.Record member (Weather_Config.Default) — do not split on '.'.
    if (metadataType === 'CustomMetadata') {
        const baseName = filePath.includes('/')
            ? filePath.split('/').pop() || fileName
            : fileName;
        return this._getDisplayMetadataName(baseName, 'CustomMetadata');
    }
