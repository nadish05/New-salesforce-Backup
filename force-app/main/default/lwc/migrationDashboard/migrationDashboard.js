...
import {
    buildLatestPlanLifecycleLookup,
    resolveLatestPlanLifecycle
} from './deploymentItemLifecycle';

import {
    BULK_INTENT_OPTIONS,
    buildBulkChangeTypeActions,
    getBulkIntentTargetIds
} from './bulkDeploymentIntent';

import {
    attachPageLayoutsToObjectGroup,
    buildUnlinkedLayoutFiles,
    extractLayoutParentObject,
    getLayoutBaseName,
    isLayoutRecord,
    recordMatchesSearch
} from './layoutObjectGrouping';

import {
    extractMetadataNameFromComparisonFile,
    isObjectFolderMetadataPath
} from './objectMetadataIdentity';

import {
    SOURCE_VALIDATION_CONTINUE_MESSAGE,
    isCoverageOnlySourceValidationFailure,
    isStage5ContinuableAfterCoverageFailure,
    resolveStage5Class,
    resolveStage5StatusClass,
    resolveStage5StatusLabel,
    shouldShowSourceValidationContinueHint
} from './sourceValidationContinuation';
...
import getDeploymentLifecycleItems
from '@salesforce/apex/DeploymentPlanController.getDeploymentLifecycleItems';

import updateLatestDeploymentItemStatuses
from '@salesforce/apex/DeploymentPlanController.updateLatestDeploymentItemStatuses';
...
import startRollbackAsync
from '@salesforce/apex/DeploymentRollbackController.startRollbackAsync';

import getRollbackStatus
from '@salesforce/apex/DeploymentRollbackController.getRollbackStatus';
...
export default class MigrationDashboard extends LightningElement {
    ...
    goToComparison() {
        ...
        this.filterLifecycleStatus = 'ALL';
    }
    ...
    /**
     * Canonical successful check-only deployment validation signal — the same
     * backend field the Ready for Deployment CTA gate reads. Enterprise summary
     * status, failure counts, and readiness reports are not substitutes.
     * Lifecycle progress only: it does not grant permission to deploy.
     */
    get isCheckOnlyValidationSuccessful() {
        return (
            this.deploymentValidationData?.checkOnlyDeployment?.success === true
        );
    }

    // Stage 5 COMPLETE only after successful check-only deployment validation
    get stage5Complete() {
        return this.isCheckOnlyValidationSuccessful;
    }

    // Stage 6 READY once check-only validation succeeded, or while viewing it
    get stage6Active() {
        return (
            this.isCheckOnlyValidationSuccessful ||
    ...
    get stage5Continuable() {
        return isStage5ContinuableAfterCoverageFailure({
            isCoverageOnlyFailure: this.isCoverageOnlySourceValidationFailure,
            isRetrievalInProgress: this.isRetrievalInProgress,
            stage5Complete: this.stage5Complete,
            stage5Active: this.stage5Active
        });
    }

    get stage5Class() {
        return resolveStage5Class({
            stage5Active: this.stage5Active,
            stage5Complete: this.stage5Complete,
            stage5Continuable: this.stage5Continuable
        });
    }
    ...
    get stage5StatusLabel() {
        return resolveStage5StatusLabel({
            stage5Active: this.stage5Active,
            stage5Complete: this.stage5Complete,
            stage5Continuable: this.stage5Continuable
        });
    ...
    get stage5StatusClass() {
        return resolveStage5StatusClass({
            stage5Active: this.stage5Active,
            stage5Complete: this.stage5Complete,
            stage5Continuable: this.stage5Continuable
        });
    ...
    latestDeploymentItems = [];

    /**
     * In-flight updateIntent promises from _applyDeploymentIntentToRecords.
     * Create Deployment Plan waits on these because Apex reads the committed
     * Selected_For_Deployment__c values. Not a second persistence path.
     */
    _pendingIntentPersistence = new Set();
    ...
    bulkDeploymentIntentOptions = BULK_INTENT_OPTIONS;
    ...
    _resetStage4To6SessionState() {
        ...
        this._validationDeploymentPlanId = null;
        this._validationLifecycleMembers = [];

        this.stopDeploymentPolling();
        this.activeDeploymentValidationId = null;
        this.deploymentStatusMessage = '';
        this._deploymentComparisonId = null;
        this._deploymentPlanId = null;
        this._deployedLifecycleMembers = null;
        ...
        this.rollbackDeploymentDateTime = null;
        this.rollbackSnapshotId = null;
        this.rollbackSnapshotPersisted = false;
        this.rollbackComponentLabels = [];
        this.isRollingBack = false;
        this.rollbackCompleted = false;
        this.stopRollbackPolling();
        this.rollbackOperationId = null;
    ...
    _loadDeploymentPlanForComparison(comparisonId) {
        ...
        const requestPlanId = this.currentDeploymentPlanId;
        ...
        return getLatestDeploymentPlanState({ comparisonId: loadComparisonId })
            .then(state => {
                ...
                // Discard if a newer plan became authoritative mid-flight
                if (this._isStalePlanResponse(requestPlanId, planId)) {
                    return null;
                }
            ...
            .catch(error => {
                ...
                return getLatestDeploymentPlanId({
                    ...
                    .catch(fallbackError => {
                        ...
                        if (this.currentDeploymentPlanId !== requestPlanId) {
                            return null;
                        }
                        this.currentDeploymentPlanId = null;
                        this.showDeploymentReview = false;
    ...
    /**
     * A plan-scoped response is stale when a different plan became current
     * while it was in flight and the response is not for that current plan.
     */
    _isStalePlanResponse(requestPlanId, responsePlanId) {
        return (
            this.currentDeploymentPlanId !== requestPlanId &&
            this.currentDeploymentPlanId !== responsePlanId
        );
    }
    ...
    /**
     * Compare stays disabled while a run is in flight and after it succeeded.
     * Both states already exist: isComparing drives the comparison animation,
     * compareCompleted is reset on comparison change and rehydrated from
     * Comparison_Status__c / saved results. A failed run leaves
     * compareCompleted false, so the button returns for a retry.
     */
    get isCompareButtonDisabled() {
        return this.isComparing || this.compareCompleted;
    }
    ...
    loadSavedComparisonResults() {
        ...
        const loadComparisonId = this.selectedComparison;
        return Promise.all([
            getComparisonResults({
                comparisonId: loadComparisonId
            }),
            getDeploymentLifecycleItems({
                comparisonId: loadComparisonId
            }).catch(error => {
                console.error('Load Lifecycle Status Error', error);
                return [];
            })
        ])
        .then(([result, deploymentItems]) => {
            if (this.selectedComparison !== loadComparisonId) {
                return;
            }
            this.savedComparisonResults = result;
            this.latestDeploymentItems = deploymentItems || [];
    ...
    /**
     * Comparison-wide lifecycle refresh on its own. Updates only the lifecycle
     * source used by buildLatestPlanLifecycleLookup, so the Stage 3 deployment
     * intent working copy, search, and filters are left untouched.
     */
    _refreshDeploymentLifecycleItems(comparisonId) {
        if (!comparisonId) {
            return Promise.resolve();
        }

        return getDeploymentLifecycleItems({ comparisonId })
            .then(deploymentItems => {
                if (this.selectedComparison !== comparisonId) {
                    return;
                }
                this.latestDeploymentItems = deploymentItems || [];
...
this.groupedComparisonResults.forEach(group => {
    expandedStates[group.groupKey || group.type] = group.expanded;
});


        const grouped = {};
        const objectFolderFiles = [];
        const layoutRecordsByObject = {};
        const unresolvedLayouts = [];
        const lifecycleLookup =
            buildLatestPlanLifecycleLookup(this.latestDeploymentItems);
        this.savedComparisonResults.forEach(record => {
            if (
                !recordMatchesSearch(record, this.searchKeyword)
            ) {
                return;
            }
            ...
            const lifecycle =
                resolveLatestPlanLifecycle(record, lifecycleLookup);
            const lifecycleFilter = this.filterLifecycleStatus || 'ALL';
            if (
                lifecycleFilter !== 'ALL' &&
                lifecycle.label !== lifecycleFilter
            ) {
                return;
            }
    ...
    showLifecycleBadge: lifecycle.showBadge,
    lifecycleLabel: lifecycle.label,
    lifecycleBadgeClass: lifecycle.cssClass,
    flowStatus: record.Flow_Status__c
};

            if (isLayoutRecord(record)) {
                const parentObject = extractLayoutParentObject(record);
                if (parentObject) {
                    if (!layoutRecordsByObject[parentObject]) {
                        layoutRecordsByObject[parentObject] = [];
                    }
                    layoutRecordsByObject[parentObject].push(enrichedFile);
                } else {
                    unresolvedLayouts.push(enrichedFile);
                }
                return;
            }

            if (isObjectFolderMetadataPath(record.File_Path__c)) {
                objectFolderFiles.push(enrichedFile);
                return;
            }
    ...
    files: []
            ...
            grouped[type].count++;
            grouped[type].files.push(enrichedFile);
        });

        if (objectFolderFiles.length) {
            const objectGroupKey = 'CustomObject';
            if (!grouped[objectGroupKey]) {
                grouped[objectGroupKey] = {
                    type: objectGroupKey,
                    groupKey: objectGroupKey,
                    count: 0,
                    expanded: expandedStates[objectGroupKey] || false,
                    files: []
                };
            }

            grouped[objectGroupKey].files.push(...objectFolderFiles);
            grouped[objectGroupKey].count =
                grouped[objectGroupKey].files.length;
        }
...
groupedResults.forEach(group => {
    ...
    group.showNestedContent =
        group.suppressTypeHeader === true || group.expanded === true;

});

this._mergeLayoutRecordsIntoGroups(
    groupedResults,
    layoutRecordsByObject,
    unresolvedLayouts,
    expandedStates
);
...
groupedResults.forEach((group) => {
    group.showNestedContent =
        group.suppressTypeHeader === true || group.expanded === true;
    group.hasRemainingFiles =
        group.type === 'ApexClass' &&
        Array.isArray(group.files) &&
        group.files.length > 0;
    group.bulkChangeTypeActions = buildBulkChangeTypeActions(
        this.savedComparisonResults,
        group.type
    );
    group.hasBulkChangeTypeActions =
        group.bulkChangeTypeActions.length > 0;
    group.bulkToolbarAriaLabel = group.hasBulkChangeTypeActions
        ? `Bulk deployment intent actions for ${group.type}`
        : '';
});
...
console.log(
    ...
    /**
     * Presentation-only — nest Layout comparison rows under their parent object.
     * Does not mutate savedComparisonResults or deployment fields.
     */
    _mergeLayoutRecordsIntoGroups(
        groupedResults,
        layoutRecordsByObject,
        unresolvedLayouts,
        expandedStates
    ) {
        const remaining = new Map(Object.entries(layoutRecordsByObject));

        const bumpCount = (group, amount) => {
            group.count = (group.count || 0) + amount;
        };

        const customGroup = groupedResults.find(
            (group) => group.type === 'CustomObject'
        );
        if (customGroup?.objectGroups) {
            customGroup.objectGroups.forEach((objectGroup) => {
                const layouts = remaining.get(objectGroup.objectName);
                if (!layouts?.length) {
                    return;
                }

                attachPageLayoutsToObjectGroup(objectGroup, layouts);
                bumpCount(customGroup, layouts.length);
                remaining.delete(objectGroup.objectName);
            });
        }

        const unlinkedLayoutFiles = buildUnlinkedLayoutFiles(
            unresolvedLayouts,
            Object.fromEntries(remaining)
        );

        if (unlinkedLayoutFiles.length > 0) {
            let layoutGroup = groupedResults.find(
                (group) => group.groupKey === 'Layout'
            );
            if (!layoutGroup) {
                layoutGroup = {
                    type: 'Layout',
                    groupKey: 'Layout',
                    count: 0,
                    expanded: expandedStates.Layout || false,
                    files: []
                };
                groupedResults.push(layoutGroup);
            }

            layoutGroup.files.push(...unlinkedLayoutFiles);
            layoutGroup.count += unlinkedLayoutFiles.length;
        }
    }

    _persistLatestPlanLifecycleStatus({
        comparisonId,
        deploymentPlanId,
        packageMembers,
        targetStatus
        ...
        if (
            !comparisonId ||
            !deploymentPlanId ||
            !Array.isArray(packageMembers) ||
            packageMembers.length === 0 ||
            this.selectedComparison !== comparisonId ||
            this.currentDeploymentPlanId !== deploymentPlanId
        ) {
            return Promise.resolve();
        }

        return updateLatestDeploymentItemStatuses({
            comparisonId,
            expectedDeploymentPlanId: deploymentPlanId,
            packageMembersJson: JSON.stringify(packageMembers),
            targetStatus
        })
            .then((items) => {
                if (
                ...
                this.latestDeploymentItems = items || [];
                this.buildGroupedResults();
            })
            .catch((error) => {
                // Lifecycle persistence must not alter validation/deploy outcome.
                console.error('Lifecycle status persistence failed', error);
            });
    }
    ...
    toggleGroup(event) {
        const selectedKey =
            event.currentTarget.dataset.groupKey ||
            event.currentTarget.dataset.type;
        this.groupedComparisonResults = this.groupedComparisonResults.map(group => {
            if ((group.groupKey || group.type) === selectedKey) {
                const expanded = !group.expanded;
                return {
                    ...group,
                    expanded,
                    showNestedContent:
                        expanded || group.suppressTypeHeader === true
                };
            }
            return group;
        });
...
toggleMetadataGroup(event) {
            ...
            if (response.success) {
                ...
                requestAnimationFrame(() => {
                    const content = this.template.querySelector(
                        '.diff-report-modal .diff-report-content'
                    );
                    if (content) {
                        content.scrollTop = 0;
                    }

                    const modalSection = this.template.querySelector(
                        '.diff-report-modal'
                    )?.closest('section.slds-modal');
                    if (modalSection) {
     ...
     * Left = Source (old / -), Right = Destination (new / +).
     */
    ...
    /**
     * Presentation-only — Lifecycle Status filter for Comparison Summary.
     */
    handleLifecycleStatusFilterChange(event) {
        const value = event?.detail?.value ?? event?.target?.value ?? 'ALL';
        this.filterLifecycleStatus = value || 'ALL';
        this.buildGroupedResults();
    }
...
/**
 * Prevent bulk intent controls from toggling the metadata group accordion.
 */
stopBulkIntentClick(event) {
    event.stopPropagation();
}

/**
 * Bulk Deploy / Ignore for all records of a metadata type + change type.
 * Operates on savedComparisonResults (not the filtered projection).
 */
handleBulkDeploymentIntentByChangeType(event) {
    event.stopPropagation();

    const metadataType = event.currentTarget.dataset.type;
    const changeType = event.currentTarget.dataset.changeType;
    const intent = event.detail?.value;
    ...
    const ids = getBulkIntentTargetIds(
        this.savedComparisonResults,
        metadataType,
        changeType
    );
    ...
    this._applyDeploymentIntentToRecords(ids, intent);
}
...
_applyDeploymentIntentToRecords(recordIds, rawValue) {
    ...
    this._trackIntentPersistence(persistence);

    return persistence;
}

/**
 * Keeps the existing updateIntent promise observable until it settles so
 * Create Deployment Plan can wait for the committed selection.
 */
_trackIntentPersistence(persistence) {
    this._pendingIntentPersistence.add(persistence);

    const release = () => {
        this._pendingIntentPersistence.delete(persistence);
    };

    persistence.then(release, release);
}

/**
 * Resolves true only when every tracked intent DML has settled successfully.
 * A false result means the selection state is uncertain.
 */
_awaitPendingIntentPersistence() {
    if (this._pendingIntentPersistence.size === 0) {
        return Promise.resolve(true);
    }

    return Promise.all([...this._pendingIntentPersistence]).then((results) =>
        results.every((succeeded) => succeeded !== false)
    );
}
...
handleCreateDeploymentPlan() {
    ...
    const createComparisonId = this.selectedComparison;

    // Apex reads committed Selected_For_Deployment__c, so the plan must not be
    // created while intent DML is still in flight.
    return this._awaitPendingIntentPersistence()
    .then(intentPersisted => {

        if (!intentPersisted) {
            // Selection state is uncertain — the intent path already reverted
            // the affected rows and reported the failure.
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Deployment Plan Not Created',
                    message:
                        'Deployment Intent could not be saved. Reapply your selection and try again.',
                    variant: 'error',
                    mode: 'dismissable'
                })
            );
        ...
        if (this.selectedComparison !== createComparisonId) {
            return null;
        }
    ...
    .then(planId => {
        ...
        if (!planId || this.selectedComparison !== createComparisonId) {
            return;
        }
        ...
        // The Stage 3 working copy stays authoritative here: a cacheable
        // comparison reload must never re-assert a stale intent over it.
        // Only the comparison-wide lifecycle source is refreshed.
        this._refreshDeploymentLifecycleItems(createComparisonId);
...
_validationDeploymentPlanId = null;

_validationLifecycleMembers = [];
...
/**
 * Stage 5 — presentation-only active tab for the Validation Details workspace.
 * Changing this only changes which existing detail section is visible.
 */
activeStage5Tab = 'failureResolution';
...
/** Async deployment execution transport — dedicated; not validation or rollback. */
deploymentPollingInterval;
activeDeploymentValidationId = null;
deploymentStatusMessage = '';
_deploymentComparisonId = null;
_deploymentPlanId = null;
_deployedLifecycleMembers = null;
...
rollbackDeploymentDateTime = null;
rollbackSnapshotId = null;
rollbackSnapshotPersisted = false;
rollbackComponentLabels = [];
isRollingBack = false;
rollbackCompleted = false;
rollbackOperationId = null;
rollbackPollingInterval = null;
...
get isCoverageOnlySourceValidationFailure() {
...
get showSourceValidationContinueHint() {
    return shouldShowSourceValidationContinueHint({
        sourceValidationData: this.sourceValidationData,
        overallSourceValidationStatus: this.overallSourceValidationStatus,
        isLoadingSourceValidation: this.isLoadingSourceValidation,
        hasDisplayItems: this.showSourceValidationCard
    });
}

get sourceValidationContinueMessage() {
    return SOURCE_VALIDATION_CONTINUE_MESSAGE;
}
...
get showRollbackPanel() {
    return (
        this.deployCompleted === true &&
        this.rollbackSnapshotPersisted === true &&
        !!this.rollbackSnapshotId
    );
}

get isRollbackDisabled() {
    return (
        this.isRollingBack ||
...
get rollbackButtonLabel() {
    return this.isRollingBack ? 'Rolling back...' : 'Rollback';
}

get rollbackDeploymentDateTimeDisplay() {
    if (!this.rollbackDeploymentDateTime) {
        return '—';
    }

    const parsed = new Date(this.rollbackDeploymentDateTime);

    if (Number.isNaN(parsed.getTime())) {
        return this.rollbackDeploymentDateTime;
    }
...
/**
 * Stage 5 — Validation Details tab definitions.
 * Availability reuses the existing section visibility flags only; no new
 * business conditions are introduced and no data is duplicated.
 */
get stage5TabDefinitions() {
    const intelligence = this.deploymentIntelligenceDisplay;
    const hasIntelligence = this.showDeploymentIntelligence;
    ...
    return [
        ...
        {
            id: 'dependencyValidation',
            label: 'Dependency Validation',
            available: true
        },
        {
            id: 'failureResolution',
            label: 'Failure Analysis & Resolution',
            available: hasIntelligence
        },
        {
            ...
            available:
                hasIntelligence &&
                (
                    !!intelligence?.hasAutoFixSection ||
                    !!intelligence?.hasAutoValidationSection
                )
        },
        {
            id: 'aiResolution',
            label: 'AI Resolution',
            available: hasIntelligence && !!intelligence?.showAiSection
        },
        {
            id: 'deploymentPlanner',
            label: 'Deployment Planner',
            available: this.showDeploymentPlanner
        },
        {
            id: 'deploymentReadiness',
            label: 'Deployment Readiness',
            available: this.showDeploymentReadinessReport
        },
        {
            id: 'aiAdvisor',
            label: 'AI Deployment Advisor',
            available: this.showAiAdvisorCard || this.isLoadingAiAdvisor
        }
    ];
}

get activeStage5TabId() {
    const availableTabs = this.stage5TabDefinitions.filter((tab) => tab.available);

    if (availableTabs.length === 0) {
        return null;
    }

    const selected = availableTabs.find((tab) => tab.id === this.activeStage5Tab);

    return selected ? selected.id : availableTabs[0].id;
}

get stage5Tabs() {
    const activeId = this.activeStage5TabId;

    return this.stage5TabDefinitions
...
get hasStage5Tabs() {
    return this.stage5Tabs.length > 0;
}
...
get stage5TabPanels() {
    ...
    this.stage5TabDefinitions.forEach((tab) => {
        const isActive = tab.id === activeId;

        panels[tab.id] = {
            cssClass: isActive
                ? 'stage5-tab-panel stage5-tab-panel_active'
                : 'stage5-tab-panel stage5-tab-panel_hidden',
...
/**
 * Metadata Validation renders a lightning-datatable, which cannot size its
 * columns correctly while its container is display:none. It carries no user
 * state, so it is rendered only while its tab is active.
 */
get isMetadataValidationTab() {
    return this.activeStage5TabId === 'metadataValidation';
}

/**
 * Gate for the Stage 5 Ready for Deployment CTA.
 * checkOnlyDeployment.success is the backend's canonical successful check-only
 * deployment validation signal; enterprise summary status, failure counts, and
 * readiness status are not substitutes for it.
 */
get isReadyForDeploymentCtaDisabled() {
    return (
        this.deploymentValidationData?.checkOnlyDeployment?.success !== true
    );
}

handleStage5TabChange(event) {
    const tabId = event.currentTarget?.dataset?.tab;

    if (tabId) {
        this.activeStage5Tab = tabId;
    }
}
...
get isDeployDisabled() {
    return (
        this.isReadyForDeploymentCtaDisabled ||
        this.isDeploying ||
        this.isLoadingDeploymentValidation
    );
}
...
handleReviewDeployment() {
    ...
    const reviewPlanId = this.currentDeploymentPlanId;
    ...
    .then(result => {
        ...
        // Also ignore a prior plan's Review once a newer plan is current.
        if (
            this.selectedComparison !== reviewComparisonId ||
            this.currentDeploymentPlanId !== reviewPlanId
        ) {
            this.isLoadingDeploymentReview = false;
            this.showIgnoredDependencyModal = false;
            this.ignoredConflicts = [];
...
handleValidateDeployment() {
    ...
    const validationDeploymentPlanId = this.currentDeploymentPlanId;
    const validationLifecycleMembers =
        (deploymentPackage.selectedMetadata || []).map((member) => ({
            ...member
        }));
    ...
    this._validationDeploymentPlanId = validationDeploymentPlanId;
    this._validationLifecycleMembers = validationLifecycleMembers;
...
_applyDeploymentValidationSuccessResponse(
    ...
    const lifecyclePlanId = this._validationDeploymentPlanId;
    const lifecycleMembers = this._validationLifecycleMembers;
    const checkOnlySuccess = response?.checkOnlyDeployment?.success;

    if (checkOnlySuccess === true) {
        this._persistLatestPlanLifecycleStatus({
            comparisonId: validationComparisonId,
            deploymentPlanId: lifecyclePlanId,
            packageMembers: lifecycleMembers,
            targetStatus: 'Validated'
        });
    } else if (checkOnlySuccess === false) {
        this._persistLatestPlanLifecycleStatus({
            comparisonId: validationComparisonId,
            deploymentPlanId: lifecyclePlanId,
            packageMembers: lifecycleMembers,
...
handleConfirmDeploy() {
    ...
    const deploymentComparisonId = this.selectedComparison;
    const deploymentPlanId = this.currentDeploymentPlanId;
    const deployedLifecycleMembers =
        (deploymentPackage.selectedMetadata || []).map((member) => ({
            ...member
        }));

    this.isDeploying = true;
    this._deploymentComparisonId = deploymentComparisonId;
    this._deploymentPlanId = deploymentPlanId;
    this._deployedLifecycleMembers = deployedLifecycleMembers;
    ...
    .then(result => {
        if (this.selectedComparison !== deploymentComparisonId) {
            this.stopDeploymentPolling();
            this.activeDeploymentValidationId = null;
            this.deploymentStatusMessage = '';
            this._deploymentComparisonId = null;
        ...
        if (
            ...
            this._handleDeploymentTransportFailure(
                startResponse?.error ||
        ...
        this.activeDeploymentValidationId = validationId;
        this.deploymentStatusMessage = 'Deployment running…';
        this._startDeploymentPolling();
    })
    .catch(error => {
        console.error(error);
...
/**
 * Dedicated deployment poll cleanup — does not touch validation or rollback timers.
 */
...
checkDeploymentStatus() {
    const validationId = this.activeDeploymentValidationId;
    if (!validationId || !this.isDeploying) {
        return;
    }

    const deploymentComparisonId = this._deploymentComparisonId;
    const deploymentContext = {
        deploymentComparisonId,
        deploymentPlanId: this._deploymentPlanId,
        deployedLifecycleMembers: this._deployedLifecycleMembers
    };

    getValidationStatus({ validationId })
        .then(result => {
            if (
                this.selectedComparison !== deploymentComparisonId ||
                this.activeDeploymentValidationId !== validationId
            ) {
                return;
            }
            ...
            const response = JSON.parse(result);
            const responseValidationId = response?.validationId;
            if (
                responseValidationId &&
                responseValidationId !== validationId
            ) {
                return;
            }
            ...
            if (status === 'COMPLETED') {
                ...
                const deploymentResult = response?.result;
                if (
                    !deploymentResult ||
                    typeof deploymentResult !== 'object'
                ) {
                    this._handleDeploymentTransportFailure(
                        'Deployment completed but returned no result.'
                    );
                    return;
                }

                this._processDeploymentCompletionResponse(
                    deploymentResult,
                    deploymentContext
                );
                return;
            ...
            if (notFound || !status) {
                this._handleDeploymentTransportFailure(
                    response?.error || 'Deployment job not found.'
                );
            }
        })
        .catch(error => {
...
_processDeploymentCompletionResponse(response, deploymentContext) {
    const {
        deploymentComparisonId,
        deploymentPlanId,
        deployedLifecycleMembers
    } = deploymentContext || {};
    ...
    if (response?.deploymentExecution?.success === true) {
        this._persistLatestPlanLifecycleStatus({
            comparisonId: deploymentComparisonId,
            deploymentPlanId,
            packageMembers: deployedLifecycleMembers,
    ...
    if (isSuccess) {
        this.deployCompleted = true;
        this._captureRollbackContext(response, deployedLifecycleMembers);
        this.showIgnoredAutoIncludedModal = false;
...
_handleDeploymentTransportFailure(message) {
    this.stopDeploymentPolling();
    this.activeDeploymentValidationId = null;
    this.deploymentStatusMessage = '';
    this._deploymentComparisonId = null;
...
_captureRollbackContext(response, deployedMembers) {
    const history = response?.deploymentHistory || {};
    const persistence = response?.snapshotPersistence || {};

    this.rollbackDeploymentId =
        history.historyId || history.deploymentId || null;
    this.rollbackDeploymentDateTime =
        history.completedAt || history.startedAt || null;
    this.rollbackSnapshotPersisted = persistence.success === true;
    this.rollbackSnapshotId = this.rollbackSnapshotPersisted
        ? (persistence.snapshotId || history.snapshotId || null)
        : (history.snapshotId || null);
    this.stopRollbackPolling();
    this.rollbackOperationId = null;
    this.rollbackCompleted = false;

    if (this.deploymentReviewItems?.length) {
        this.rollbackComponentLabels = this.deploymentReviewItems.map((item) => ({
            key: item.key,
            label: item.displayMetadataName || item.metadataName
        }));
        return;
    }

    this.rollbackComponentLabels = (deployedMembers || []).map((member, index) => ({
        key: `${member.metadataType || member.type || 'member'}-${
            member.metadataName || member.name || index
        }`,
        label: member.metadataName || member.name || ''
    }));
}

_parseRollbackResponse(raw) {
    if (raw == null) {
        return {
            kind: 'error',
            message: 'Rollback returned no response.',
            code: null
        };
    }
    ...
    if (status === 'IN_PROGRESS') {
        return {
            kind: 'inProgress',
            message,
            code: parsed.resultCode || parsed.code || null,
    ...
    if (status === 'UNKNOWN_RESULT') {
        return {
            ...
            message:
                parsed.error ||
                'Rollback result could not be confirmed.',
            code: parsed.resultCode || parsed.code || null
        };
    }
    ...
    if (parsed.blocked === true) {
        return { kind: 'blocked', message, code: parsed.code || null };
    }

    if (parsed.failed === true) {
        return { kind: 'failed', message, code: parsed.code || null };
    }

    if (parsed.unknownResult === true) {
        return { kind: 'unknownResult', message, code: parsed.code || null };
    }

    if (parsed.success === false || parsed.error) {
        return { kind: 'error', message, code: parsed.code || null };
    }

    return { kind: 'unknownResult', message, code: parsed.code || null };
}

_dispatchRollbackResultToast(outcome) {
    const code = outcome.code || '';
    const message = outcome.message || 'Rollback request completed.';
    ...
    if (
        ...
        code === 'ROLLBACK_ALREADY_IN_PROGRESS'
    ) {
        this.dispatchEvent(
            new ShowToastEvent({
...
handleRollback() {
    ...
    if (!destinationOrg) {
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'Rollback failed',
                message: 'Destination org details not available.',
                variant: 'error',
                mode: 'dismissable'
            })
        );
    ...
    startRollbackAsync({
        snapshotId: this.rollbackSnapshotId,
        historyId: this.rollbackDeploymentId,
        refreshToken: destinationOrg.Refresh_Token__c,
        instanceUrl: destinationOrg.Instance_URL__c,
        orgId: destinationOrg.Org_ID__c
    })
    .then((raw) => {
        ...
        if (!operationId) {
            ...
            this.rollbackOperationId = null;
            this.isRollingBack = false;
            this._dispatchRollbackResultToast(
                this._parseRollbackResponse(raw)
            );
            return;
        }
...
startRollbackPolling() {
    ...
    const operationId = this.rollbackOperationId;
    if (!operationId) {
        return;
    }

    this.checkRollbackStatus(operationId);
    this.rollbackPollingInterval = setInterval(() => {
        this.checkRollbackStatus(operationId);
    }, 5000);
}

stopRollbackPolling() {
    if (this.rollbackPollingInterval) {
        clearInterval(this.rollbackPollingInterval);
        this.rollbackPollingInterval = null;
    }
}

checkRollbackStatus(operationId) {
    if (!operationId || operationId !== this.rollbackOperationId) {
        return;
    }

    getRollbackStatus({ operationId })
    .then((raw) => {
        if (operationId !== this.rollbackOperationId) {
            return;
        }

        let responsePayload = raw;
        if (typeof raw === 'string') {
            try {
        ...
        const responseOperationId =
            ...
            typeof responsePayload.operationId === 'string'
                ? responsePayload.operationId.trim()
                : '';
        ...
        const outcome = this._parseRollbackResponse(raw);
        if (outcome.kind === 'inProgress') {
            this.isRollingBack = true;
            return;
        }
        ...
        this._dispatchRollbackResultToast(outcome);
    })
    .catch((error) => {
        if (operationId !== this.rollbackOperationId) {
            return;
        ...
        this.dispatchEvent(
            new ShowToastEvent({
                ...
                message:
                    error?.body?.message ||
                    error?.message ||
                    'Unable to retrieve rollback status.',
                variant: 'error',
                mode: 'sticky'
            })
        );
...
_extractMetadataName(file) {
    ...
    if (metadataType === 'Layout' || fileName.endsWith('.layout-meta.xml')) {
        return getLayoutBaseName(file);
    }

    const resolvedName = extractMetadataNameFromComparisonFile(file);
    if (resolvedName) {
        return resolvedName;
    }
...
_getDisplayMetadataName(fileName, metadataType) {
    ...
    if (type === 'Layout') {
        return fileName.replace(/\.layout-meta\.xml$/i, '');
    }
