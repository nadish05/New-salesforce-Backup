...
export default class MigrationDashboard extends LightningElement {
    ...
    // Stages 1–3 unchanged. Stages 4–6 always navigable in Phase 1.
    ...
    get stage4Disabled() { return false; }
    get stage5Disabled() { return false; }
    get stage6Disabled() { return false; }
    ...
    /**
     * Restores currentDeploymentPlanId from Salesforce for the selected comparison.
     * Does not create or modify plans. Leaves review/validation payloads empty.
     */
    _loadDeploymentPlanForComparison(comparisonId) {
        ...
        return getLatestDeploymentPlanId({ comparisonId })
            .then(planId => {
                this.currentDeploymentPlanId = planId || null;
                // Unlock Stage 4 Review action when a durable plan exists (same as create-plan path)
 ...
 * Reuses Apex updateIntent + working-copy sync + reload re-assert.
...
_applyDeploymentIntentToRecords(recordIds, rawValue) {
    ...
    const applyIntentToWorkingCopy = () => {
        this.savedComparisonResults =
    ...
    return Promise.all(
        ...
        .then(() => {
            // Immediate client sync — same rules as Apex updateIntent.
            applyIntentToWorkingCopy();

            // Keep existing reload for eventual consistency with Salesforce.
            return this.loadSavedComparisonResults();
        })
        .then(() => {
            // Re-assert working-copy sync if cacheable reload returned a stale snapshot.
            applyIntentToWorkingCopy();
        })
        .catch((error) => {
            console.error(error);
...
handleValidateDeployment() {
    ...
    .then(result => {
        ...
        // Presentation only — open once after processing; never if empty/null.
        this.showIgnoredAutoIncludedModal = false;
        if (this.hasIgnoredAutoIncludedMetadata) {
        ...
        // Phase 10F — advisory only; never blocks planner / deploy workflow.
...
_buildOnDemandAiResolutionContext() {
    const data = this.deploymentValidationData;
...
/**
 * Presentation-only on-demand AI result. Backend SAFE_SKIP remains authoritative.
 */
